...
The orchestrator running this playbook stays lean: it reads cards, dispatches subagents, and collects their output. It never writes application code itself. Every dispatch follows the dispatch brief in `playbooks/system.md` "The agent tree" — objective, inputs by path (never pasted bodies), the typed output contract, boundaries — and every return is parsed fields plus artifacts on disk, never a transcript. At every wave boundary the orchestrator writes `STATE.md` at wave granularity (epic, wave just finished, next wave, pointers), so a fresh session can resume mid-epic at any boundary — externalizing state is routine, never a reaction to context pressure. And at each of those boundaries it runs the predictive budget check from `playbooks/system.md` "The context budget": estimate the next wave's footprint, and if current fill plus that estimate crosses roughly 60% — or fill alone is already past 40–50% — finish the boundary write and close before launching anything. The fresh session resumes and loses nothing. Never ride a session toward auto-compaction; quality degrades from mid-window, and compaction fires at the model's weakest moment.
...
**The worker relay.** A worker that nears its own context budget mid-story stops at a clean point — code compiling, nothing half-edited — and writes its typed report with `status: PARTIAL`: files touched so far, dead ends, and a `remaining:` field concrete enough for a stranger to finish from. The orchestrator dispatches one fresh continuation worker with three inputs: the card, `CONTRACTS.md`, and the partial report. The continuation worker finishes the story, runs the acceptance check, and writes the final report. One relay maximum — a second PARTIAL on the same story is a slicing defect (two stories wearing one card): flag it and route it back to slicing, exactly like NOT-WORKER-READY. Workers never dispatch their own successors.
...
- **Parse every `stories/SN.report.md`** — the typed fields, not the prose. Five conditions get flagged mechanically, no reading-between-the-lines required: `status: FAIL`, `NOT-WORKER-READY`, or `PARTIAL` (a PARTIAL reaching Wave 2 means the relay never completed — an unfinished story, never merged); any `files_touched` entry outside that story's ownership list (a collision that slipped past pre-flight); a non-empty `deviations` list; a non-empty `contract_change_requests` list. `repair_rounds_used` of 2 or more is a card-quality signal — note it in `LEARNINGS.md`, because a card that needed that many repairs was under-specified at slicing time.
- **Cross-check reports against reality:** run `node <playbook-root>/../scripts/files-audit.js <epic-dir>` — it diffs the actual changed files in the working tree against declared card ownership and reported `files_touched`, so an edit a worker omitted from its report cannot hide. Any `unowned` or `unreported` entry is flagged exactly like an out-of-ownership `files_touched` entry.
