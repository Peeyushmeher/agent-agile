...
- **An epic fails verification twice in a row, even after a redo.** Stop the autopilot loop entirely. Write a `STATE.md` update recording exactly where the run stopped and why, so a human (or a resumed session) has full context without re-deriving it. This is what prevents building further epics on top of one that's still broken.
