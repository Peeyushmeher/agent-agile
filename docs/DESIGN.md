...
**The context budget** (same doctrine section): quality degrades from ~40–50% window fill, driven by junk density, and auto-compaction fires at the model's weakest moment — so the orchestrator's boundary check is **predictive** (hand off before launching any wave whose estimated footprint would push fill past ~60%, even from 35%; STATE.md is wave-granular so a resumed session loses nothing), workers are bounded by construction (card+contracts in, typed report out — a worker grinding deep into its window is a slicing defect), a worker that fills anyway stops clean and reports `PARTIAL` for a **one-relay-max orchestrator-dispatched continuation** (never a self-spawned successor — relays sequence through the orchestrator, keeping the tree flat), and compaction is the emergency brake, never the plan. (Grounded in Chroma's context-rot study, NoLiMa/RULER effective-length data, SWE-bench long-context ablations, Anthropic's harness-design post on context anxiety and reset-over-compaction, and converging practitioner thresholds.)
