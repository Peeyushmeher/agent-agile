...
- Check context fill at every wave and epic boundary (the context budget in `playbooks/system.md` "The agent tree"). Past roughly 40–50%: write `STATE.md` at the boundary and stop cleanly for a fresh-session resume. This is routine hygiene, not a circuit breaker — a resumed loop is identical to one that never stopped, while a loop that rides to auto-compaction finishes its roadmap with the model at its weakest.
