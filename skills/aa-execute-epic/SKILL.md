...
3. **Wave 1 — stories.** Spawn one fresh `aa-worker` subagent per story card, in parallel, each with the model configured for the cheap tier in `.planning/CONFIG.md`. Give each worker exactly two inputs — its own story card and `CONTRACTS.md` — nothing else: no other story's card, no wider exploration mandate. Each worker builds against its `Files it owns` list, runs its own acceptance check, and writes `epics/EPIC-NN/stories/SN.report.md`. If a worker's acceptance check fails, give it exactly one retry within the same dispatch; if it fails again, flag the story and do not merge its output.
